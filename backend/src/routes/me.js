...
export default function meRoutes(db) {
   ...
   *
   * The foods this client logs REPEATEDLY, most-logged first.
   *
   * Logging a meal cost a sheet, a search, a portion and a confirm --
   * every time, including for the same porridge eaten every weekday
   * morning. Saved Foods existed but required you to have curated it
   * first, which is work done up front for a benefit you cannot yet see,
   * so most people never did. This needs no curation: eat something
   * twice and it shows up.
   *
   * Grouped case-insensitively on name, because "Chapati" and "chapati"
   * are the same breakfast and splitting them halves both counts and
   * hides the thing from the top of the list. The macros returned are
   * the MOST RECENT ones logged under that name rather than an average:
   * a portion you actually ate is a real quantity someone can recognise,
   * an average of three portions is a number that was never on a plate.
   */
  r.get('/nutrition/frequent', async (req, res) => {
    const c = await getClient(req, res); if (!c) return;
    const limit = Math.min(Math.max(parseInt(req.query.limit, 10) || 8, 1), 20);
    const tz = req.tz || 'Asia/Kolkata';
    const since = new Date(Date.now() - 45 * 86400000).toISOString().slice(0, 10);

    // PORTABLE ON PURPOSE. `SELECT name ... GROUP BY LOWER(name)` is legal
    // in SQLite (it picks an arbitrary row) and a hard ERROR in Postgres,
    // where a selected column must be grouped or aggregated -- so the
    // tests, which run on SQLite, would have passed while production 500'd.
    // MAX(name) picks one spelling deterministically instead.
    const rows = await db.q(
      `SELECT MAX(name) AS name, COUNT(*) AS times, MAX(date) AS last_date
         FROM meal_logs
        WHERE client_id = ? AND eaten = 1 AND date >= ?
        GROUP BY LOWER(name)
       HAVING COUNT(*) >= 2
        ORDER BY COUNT(*) DESC, MAX(date) DESC
        LIMIT ${limit}`, [c.id, since]);

    // One extra query per food, but bounded at 20 and only for foods the
    // client has eaten at least twice -- and it is the only way to return
    // the portion they last actually had rather than an invented average.
    const out = [];
    for (const r0 of rows) {
      const last = await db.q1(
        `SELECT name, calories, protein, carbs, fat, quantity, unit, unit_type, source
           FROM meal_logs
          WHERE client_id = ? AND eaten = 1 AND LOWER(name) = LOWER(?)
          ORDER BY date DESC LIMIT 1`, [c.id, r0.name]);
      if (!last) continue;
      out.push({
        name: last.name,
        times: Number(r0.times),
        lastDate: r0.last_date,
        calories: Math.round(Number(last.calories) || 0),
        protein: Number(last.protein) || 0,
        carbs: Number(last.carbs) || 0,
        fat: Number(last.fat) || 0,
        quantity: last.quantity ?? null,
        unit: last.unit ?? null,
        unitType: last.unit_type ?? null,
        // Carried so a repeat inherits the ORIGINAL's provenance. Re-logging
        // an AI-estimated food does not turn its numbers into measured ones,
        // and mealLog's `source` is a closed enum anyway -- inventing a
        // 'quick_repeat' value would simply 422.
        source: last.source || 'manual',
      });
    }

    res.json({ foods: out, since, today: await logToday(c.id, tz) });
  });
