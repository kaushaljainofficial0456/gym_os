...
export default function Progress() {
  ...
  const logWeight = async () => {
    ...
    // the server first. Merged with main's redesigned {text, tone} toast
    // shape (this file's toast rendering expects that object, not a bare
    // string) while keeping every case the original fix covered --
    // main's own validation only caught blank/0/negative, not NaN or an
    // over-500 typo.
    const raw = weightInput.trim();
    if (!raw) { setToast({ text: 'Enter a weight first', tone: 'bad' }); return; }
    const w = parseFloat(raw);
    if (!Number.isFinite(w) || w <= 0) { setToast({ text: 'Enter a valid weight greater than 0', tone: 'bad' }); return; }
    if (w > 500) { setToast({ text: 'That weight looks too high — double-check it', tone: 'bad' }); return; }
    setSaving(true);
    try {
