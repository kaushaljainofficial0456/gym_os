/**
 * NutritionTargetSetup — dual-mode component:
 *
...
// calories its own independently-editable field alongside protein/carbs/
// fat -- a deliberate product change (the user can now set a calorie
// target that doesn't algebraically match 4/4/9 of the macros, same as
// several real nutrition apps do), not something for a merge to silently
// revert back to a derived value.
...
export default function NutritionTargetSetup({ open, onComplete, currentPlan = null, isEdit = false }) {
  ...
  const handleSave = async () => {
    ...
    if (!calories || calories < 500 || calories > 10000) {
      setSaveError('Please enter a valid daily calorie target (500–10,000).');
      return;
    }
    if (!protein || protein < 20 || protein > 500) {
      setSaveError('Please enter valid protein (20–500g).');
      return;
    }
    if (!carbs || carbs < 20 || carbs > 800) {
      setSaveError('Please enter valid carbs (20–800g).');
      return;
    }
    if (!fat || fat < 15 || fat > 300) {
      setSaveError('Please enter valid fat (15–300g).');
      return;
    }
    ...
    try {
      if (isEdit) {
        // Update existing plan — mark as manual
        ...
        // First-time setup — create new plan
  ...
  const handleReset = async () => {
    ...
    setSaveError(null);
    try {
      // Reset to calculated values — mark as automatic
      await api('/me/nutrition/targets', {
        method: 'PUT',
        body: JSON.stringify({
        ...
        <div className="px-6 py-5">
          {loading && (
            <div className="text-center py-10">
              ...
              <div className="text-[11px]" style={{ color: t.mute }}>Loading targets…</div>
            </div>
          )}
          ...
          {targets && !loading && !confirmed && (
            <div className="space-y-4">
              ...
              {/* All macros — editable including calories */}
        ...
        {targets && !loading && !confirmed && (
          <div className="px-6 pb-6 space-y-2">
            <button onClick={handleSave} disabled={saving}
              ...
              {saving ? 'Saving…' : isEdit ? 'Save Changes' : 'Confirm Targets'}
            </button>

            {/* Reset to calculated — only shown in edit mode when plan is manual */}
            {isEdit && isManual && (
