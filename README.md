...
- **Super Admin:** `SUPER_ADMIN` has a dedicated UI — the Admin Console, a genuinely separate app (`admin/`, own `package.json`/build/deploy, distinct from `frontend/`) calling `/api/console/*` (`backend/src/routes/console.js`). No self-serve signup exists for this role by design — bootstrap one with `node backend/scripts/create-super-admin.js --email you@example.com --password '...'`. Covers: platform-wide dashboard KPIs, gym list/detail with suspend/reactivate, cross-org payments, reconciliation, refunds, PDF invoices, support tickets (incl. internal notes), Food Intelligence, fraud/risk monitoring, ML monitoring (calorie model), feature flags, platform announcements, an error center + system health view, CSV data export, and the audit log. Feature flags and announcements are built and evaluable but not yet adopted by any real feature/frontend surface; `ORG_PACKAGE` refunds need an additive schema migration first.